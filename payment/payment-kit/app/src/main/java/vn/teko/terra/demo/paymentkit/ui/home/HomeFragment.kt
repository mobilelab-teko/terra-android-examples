...
import vn.teko.terra.demo.paymentkit.utils.DefaultHestiaCallback
...
class HomeFragment : Fragment() {
    ...
    private fun openNativeAppWithSelfPayment() {
        val hestia = TerraHestia.getInstance(terraApp = terraApp)
        hestia.startApp("mini_app", callback = DefaultHestiaCallback(requireContext()))
    }

    private fun openNativeAppWithHostPayment() {
        val hestia = TerraHestia.getInstance(terraApp = terraApp)
        hestia.startApp("mini_app_02", callback = DefaultHestiaCallback(requireContext()))
    }
